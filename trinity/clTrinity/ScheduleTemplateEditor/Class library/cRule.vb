Public Class cRule
    ...
    Public ReadOnly Property Template() As cScheduleTemplate
        Get
            Return _template
        End Get
    End Property
    ...
    Function Clone(Template As cScheduleTemplate) As cRule
        Dim _rule As New cRule(Template)
        For Each _search In Searches.Where(Function(s) s.ParentSearch Is Nothing)
            _search.Clone(_rule, Template)
        Next
        Return _rule
    End Function

    Sub Validate(Schedule As cSchedule)
        _validationResult = New cSearchResult With {.Succeeded = False}
        For Each _search In Searches
    ...
    Public Sub New(Template As cScheduleTemplate)
        _template = Template
    End Sub
End Class
